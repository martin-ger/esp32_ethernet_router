...
// Number of consecutive link-down readings required before reporting link down.
// A single bad PHYCFGR SPI read (noise, glitch) would otherwise trigger a
// spurious link-down/up cycle.  Link-up is still reported immediately.
#define W5500_LINK_DOWN_DEBOUNCE 4
...
typedef struct {
    ...
    int reset_gpio_num;
    uint8_t link_down_count;   // consecutive link-down readings for debounce
} phy_w5500_t;
...
static esp_err_t w5500_update_link_duplex_speed(phy_w5500_t *w5500)
{
    ...
    // Debounce link-down: require W5500_LINK_DOWN_DEBOUNCE consecutive readings
    // before reporting.  A single bad SPI read (noise/glitch) would otherwise
    // cause a spurious link-down/up cycle, especially during idle periods.
    // Link-up is reported immediately (no debounce needed).
    if (link == ETH_LINK_DOWN) {
        if (w5500->link_down_count < W5500_LINK_DOWN_DEBOUNCE) {
            w5500->link_down_count++;
            return ESP_OK;  // not yet confirmed, ignore this reading
        }
    } else {
        w5500->link_down_count = 0;
    }
