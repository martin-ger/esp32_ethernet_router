 ...
 * auto-discovery. Compile-time guarded by CONFIG_MQTT_HOMEASSISTANT.
 *
...
/* ---------- MQTT topics ---------- */
#define TOPIC_PREFIX        "esp32_eth_router"
...
#define TOPIC_WOL_PREFIX    TOPIC_PREFIX "/wol/"
#define TOPIC_WOL_SUFFIX    "/action"
...
static char s_device_id[32];   /* "esp32_eth_router_AABBCCDDEEFF" */
...
static void build_device_id(void)
{
    ...
    snprintf(s_device_id, sizeof(s_device_id),
             "esp32_eth_router_%s", s_mac_str);
}

static int device_json(char *buf, size_t len)
{
    char ip_str[16];
    uint32_t cfg_ip = my_ap_ip != 0 ? my_ap_ip : my_ip;
    ...
    return snprintf(buf, len,
        ...
        "\"sw\":\"%s\","
        "\"cu\":\"http://%s\"}",
        s_device_id, hostname && hostname[0] ? hostname : "esp32-eth-router",
...
static void publish_discovery(void)
{
    ...
    /* button: Wake-on-LAN — one per named DHCP reservation */
    for (int i = 0; i < MAX_DHCP_RESERVATIONS; i++) {
        if (!dhcp_reservations[i].valid) continue;
        if (!dhcp_reservations[i].name[0]) continue;

        const char *name = dhcp_reservations[i].name;
        snprintf(topic, sizeof(topic),
            "homeassistant/button/%s/wol_%d/config", s_device_id, i);
        snprintf(payload, sizeof(payload),
            "{\"name\":\"Wake %s\","
            "\"uniq_id\":\"%s_wol_%d\","
            "\"cmd_t\":\"" TOPIC_WOL_PREFIX "%s" TOPIC_WOL_SUFFIX "\","
            "\"ic\":\"mdi:power\","
            "\"avty_t\":\"" TOPIC_AVAILABILITY "\","
            "%s}", name, s_device_id, i, name, dev);
        esp_mqtt_client_publish(s_client, topic, payload, 0, 1, 1);
    }
                                ...
                                int32_t event_id, void *event_data)
{
    ...
    case MQTT_EVENT_CONNECTED:
        ...
        /* Single-level wildcard to receive WoL commands for any device name */
        esp_mqtt_client_subscribe(s_client, TOPIC_WOL_PREFIX "+" TOPIC_WOL_SUFFIX, 1);
        publish_discovery();
        publish_state(NULL);
        break;
    ...
    case MQTT_EVENT_DATA:
        if (event->topic_len > 0 && event->topic != NULL) {
            ...
            if (event->topic_len == (int)strlen(TOPIC_CMD_RC) &&
                ...
                if (on) remote_console_enable(); else remote_console_disable();
                publish_state(NULL);
            }
            /* WoL: topic = esp32_eth_router/wol/<name>/action */
            const int prefix_len = sizeof(TOPIC_WOL_PREFIX) - 1;
            const int suffix_len = sizeof(TOPIC_WOL_SUFFIX) - 1;
            if (event->topic_len > prefix_len + suffix_len &&
                memcmp(event->topic, TOPIC_WOL_PREFIX, prefix_len) == 0 &&
                memcmp(event->topic + event->topic_len - suffix_len,
                       TOPIC_WOL_SUFFIX, suffix_len) == 0) {
                int name_len = event->topic_len - prefix_len - suffix_len;
                char name[DHCP_RESERVATION_NAME_LEN + 1];
                if (name_len > 0 && name_len <= DHCP_RESERVATION_NAME_LEN) {
                    memcpy(name, event->topic + prefix_len, name_len);
                    name[name_len] = '\0';
                    uint8_t mac[6];
                    if (resolve_device_name_to_mac(name, mac)) {
                        wol_send_mac(mac, my_ap_ip, "255.255.255.255", 9);
                        ESP_LOGI(TAG, "WoL sent to '%s' via MQTT", name);
                    } else {
                        ESP_LOGW(TAG, "WoL: unknown device '%s'", name);
                    }
                }
            }
        }
...
static int mqtt_cmd(int argc, char **argv)
{
    ...
    if (strcmp(action, "status") == 0) {
        ...
        if (s_connected) printf("Device ID: %s\n", s_device_id);
        return 0;
    }

    if (strcmp(action, "enable") == 0) {
        s_enabled = true;
        set_config_param_int(NVS_KEY_MQTT_EN, 1);
        esp_err_t err = mqtt_ha_start();
        if (err != ESP_OK) { printf("Failed to start: %s\n", esp_err_to_name(err)); return 1; }
    ...
    if (strcmp(action, "disable") == 0) {
        s_enabled = false;
        set_config_param_int(NVS_KEY_MQTT_EN, 0);
        mqtt_ha_stop();
    ...
    if (strcmp(action, "broker") == 0) {
        if (mqtt_args.arg1->count == 0) { printf("Usage: mqtt broker <uri>\n"); return 1; }
        strncpy(s_uri, mqtt_args.arg1->sval[0], sizeof(s_uri) - 1);
        s_uri[sizeof(s_uri) - 1] = '\0';
        set_config_param_str(NVS_KEY_MQTT_URI, s_uri);
    ...
    if (strcmp(action, "user") == 0) {
        ...
        set_config_param_str(NVS_KEY_MQTT_USER, s_user);
        set_config_param_str(NVS_KEY_MQTT_PASS, s_pass);
        printf("MQTT credentials set\n");
        return 0;
    }

    if (strcmp(action, "interval") == 0) {
        if (mqtt_args.arg1->count == 0) { printf("Usage: mqtt interval <seconds>\n"); return 1; }
        int val = atoi(mqtt_args.arg1->sval[0]);
        if (val < 5 || val > 3600) { printf("Interval must be 5-3600 seconds\n"); return 1; }
        s_interval = (uint32_t)val;
        set_config_param_int(NVS_KEY_MQTT_INTV, (int32_t)s_interval);
...
/* ================================================================
 *  Init (called from app_main after console init)
 * ================================================================ */
