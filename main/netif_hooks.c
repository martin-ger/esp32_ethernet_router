 ...
 *
 * Hooks into the lwIP netif input/linkoutput chains for both STA (uplink)
 * and Ethernet downlink interfaces to intercept packets for filtering and monitoring.
 */
...
// Original netif input and linkoutput function pointers (STA uplink)
...
// Original downlink netif function pointers (Ethernet downlink)
static netif_input_fn original_dl_netif_input = NULL;
static netif_linkoutput_fn original_dl_netif_linkoutput = NULL;
static struct netif *dl_netif = NULL;
...
void resync_connect_count(void) {
    // No WiFi AP station list on Ethernet downlink — cannot resync.
    // Future: inspect DHCP lease table for active clients.
}
...
// Send ICMP Fragmentation Needed (Type 3, Code 4) back to a client on the downlink interface.
// 'p' is an inbound Ethernet frame from the downlink; 'mtu' is the next-hop MTU to report.
...
{
    if (p == NULL || netif == NULL || original_dl_netif_linkoutput == NULL) return;
    ...
    // Ethernet: swap src/dst MAC (downlink MAC -> client MAC)
    memcpy(buf + 0, eth + 6, 6); // dst = original sender's MAC
    memcpy(buf + 6, eth + 0, 6); // src = our downlink MAC (original frame's dst)
    buf[12] = 0x08;
    buf[13] = 0x00;
    ...
    *((uint16_t *)(icmp + 2)) = inet_chksum(icmp, icmp_total);
    ...
    original_dl_netif_linkoutput(netif, resp);
    pbuf_free(resp);
}
...
static IRAM_ATTR err_t dl_netif_input_hook(struct pbuf *p, struct netif *netif) {
    ...
    if (!acl_is_empty(ACL_FROM_ETH)) {
        uint8_t result = acl_check_packet(ACL_FROM_ETH, p);
    ...
    // Route-all mode: block all non-downlink-subnet traffic (prevents internet leakage)
    ...
    // Call original input function
    if (original_dl_netif_input != NULL) {
        return original_dl_netif_input(p, netif);
    }
...
static IRAM_ATTR err_t dl_netif_linkoutput_hook(struct netif *netif, struct pbuf *p) {
    ...
    if (!acl_is_empty(ACL_TO_ETH)) {
        uint8_t result = acl_check_packet(ACL_TO_ETH, p);
    ...
    // Call original linkoutput function
    if (original_dl_netif_linkoutput != NULL) {
        return original_dl_netif_linkoutput(netif, p);
    }
...
void init_downlink_netif_hooks(void) {
    if (ethNetif != NULL && original_dl_netif_input == NULL) {
        // Access internal lwIP netif - this is internal API but necessary for hooking
        extern struct netif *esp_netif_get_netif_impl(esp_netif_t *esp_netif);
        dl_netif = esp_netif_get_netif_impl(ethNetif);

        if (dl_netif != NULL) {
            // Store and hook input function
            original_dl_netif_input = dl_netif->input;
            dl_netif->input = dl_netif_input_hook;

            // Store and hook linkoutput function
            original_dl_netif_linkoutput = dl_netif->linkoutput;
            dl_netif->linkoutput = dl_netif_linkoutput_hook;

            ESP_LOGI(TAG, "Downlink netif hooks initialized for Ethernet (input & output)");
        }
    }
}
