...
int get_connected_clients(connected_client_t *clients, int max_clients) {
    ...
    // On Ethernet downlink, use DHCP lease table as the source of truth
    // (no WiFi AP station list available)
    ...
    int count = 0;
    for (int i = 0; i < lease_count && count < max_clients; i++) {
        memcpy(clients[count].mac, leases[i].mac, 6);
        clients[count].ip = leases[i].ip;
        clients[count].has_ip = true;
        clients[count].name[0] = '\0';

        // Use DHCP hostname if provided by client
        if (leases[i].hostname[0] != '\0') {
            strncpy(clients[count].name, leases[i].hostname,
        ...
        for (int j = 0; j < MAX_DHCP_RESERVATIONS; j++) {
            if (dhcp_reservations[j].valid &&
                memcmp(dhcp_reservations[j].mac, leases[i].mac, 6) == 0) {
