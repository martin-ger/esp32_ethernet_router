...
void vpn_reassert_default_route(void)
{
     ...
     * route_prio on every STA GOT_IP (initial association AND periodic DHCP
     ...
     * tunnel silently loses the default route to the STA and route-all traffic
     * leaks straight out the uplink. Re-assert the tunnel as default whenever
     * one of those events fires while the VPN is up in route-all mode.
     *
