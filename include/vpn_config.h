...
extern char* vpn_dns;               // DNS server handed to ETH clients while VPN is enabled (optional)
...
// STA GOT_IP / interface-up events, which make esp_netif reset netif_default to
// the uplink and would otherwise break route-all mode. No-op unless the VPN is
// connected with route_all enabled.
